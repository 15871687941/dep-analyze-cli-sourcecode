...
export default class DepAnalyze {
    ...
    // 多个不同版本的包的列表
    private mulPackList: object[];
    // 辅助队列
    private helpQueue: string[];

    private isLocal: boolean;

    private getDepConfigObj: (
    entryPackage: string,
    entryVersion: string,
  ) => DepConfObj;
    ...
    private readDepsGraph(ep: string, ev: string, depthLimited: number) {
      ...
      this.helpQueue.length > this.depth ? this.helpQueue.length : this.depth;
        ...
        const deps: Map<string, string> = new Map(
            Object.entries(depConfObj.dependencies),
        );
        let depName: string = '';
        let verName: string = '';
        ...
        // const i:number = 0;
        for (
            let w: string = this.depGraph.getFirstNeighbor(node) as string;
            w !== undefined;
            w = this.depGraph.getNextNeighbor(node, w) as string
        ) {
            // 即将遍历w节点
            // 解决循环依赖问题
    ...
    getDepList(): string[] {
        const depList: DepPkgVer[] = getDepPkgVerList();
        const depsString: string[] = [];
        for (const index in depList) {
            // const dep = depList[index];
            depsString.push(
                `${depList[index]['packageName']}:${depList[index].version}`,
            );
        }
        depsString.sort();
        return depsString;
    ...
    // console.log(this.depGraph);
        ...
        for (const source of this.depGraph.getNodes()) {
            for (const target of this.depGraph.getNeighbors(source) as string[]) {
